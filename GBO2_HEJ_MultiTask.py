...
def initialize_multitask_model(train_x, train_obj):
    model = MultiTaskGP(train_x, train_obj, task_feature=-1, outcome_transform=Standardize(m=1))
...
def get_mfkg(model):
    # Use ScalarizedPosteriorTransform to reduce the multitask posterior to a scalar
    posterior_transform = ScalarizedPosteriorTransform(weights=torch.tensor([1.0]))
    curr_val_acqf = FixedFeatureAcquisitionFunction(
        acq_function=PosteriorMean(model, posterior_transform=posterior_transform),
        d=3,
        columns=[2],
...
for exper in range(N_exper):
    ...
    path = "/cluster/home/mnobar/code/GBO2/logs/test_MT_29_baseline/Exper_{}".format(str(exper))
    ...
    # uncomment for my idea
    fidelities = torch.tensor([0, 1.0], **tkwargs)
    ...
    for i in range(N_ITER):
        ...
        mll, model = initialize_multitask_model(train_x, train_obj)
